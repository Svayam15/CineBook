...
// ─── Update Modal (price / showType only — no bookings allowed by backend) ───
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl">
        ...
        <div className="flex gap-3 justify-end mt-6">
          <button onClick={onClose} className="px-4 py-2 rounded-xl border border-border text-muted hover:text-white text-sm transition">
            Cancel
          </button>
          <button
      ...
      <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl">
        ...
        <div className="flex gap-3 justify-end mt-6">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-5 py-2 rounded-xl text-sm font-medium transition disabled:opacity-50"
          >
            {saving ? <Spinner /> : <CalendarClock size={14} />}
            Reschedule
          </button>
        </div>
      </div>
      ...
      {canEdit && show.isActive ? (
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-blue-400 hover:text-blue-300 border border-blue-400/20 hover:border-blue-400/40 px-3 py-1.5 rounded-xl text-sm transition"
          >
          ...
          >
            {cancelling === show.id ? <Spinner /> : <Trash2 size={13} />}
            Cancel
          </button>
        </div>
      ) : (
...
const Shows = () => {
  ...
  const [form, setForm] = useState({
    ...
    totalSeats: 120, showType: "2D", regularPrice: "",
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ movieId: "", theatreId: "", startTime: "", totalSeats: 120, showType: "2D", regularPrice: "", hasGoldenSeats: false, goldenSeats: "", goldenPrice: "" });
      await fetchData();
    } catch (err) {
      toast.error(err?.response?.data?.message || err.message);
    } finally {
  ...
  return (
    <AdminLayout>
      ...
      {reschedulingShow && (
        <RescheduleModal
          show={reschedulingShow}
          onSave={handleReschedule}
          onClose={() => setReschedulingShow(null)}
          saving={saving}
        />
      )}
        ...
        <div className="bg-card border border-border rounded-2xl p-5 mb-6">
          ...
          <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.theatreId} onChange={(e) => setForm({ ...form, theatreId: e.target.value })} required
                className="w-full bg-dark border border-border text-white rounded-xl px-4 py-2.5 outline-none focus:border-primary text-sm">
            ...
            <div>
              <label className="block text-sm text-muted mb-1.5">Show Type</label>
              <select value={form.showType} onChange={(e) => setForm({ ...form, showType: e.target.value })}
                className="w-full bg-dark border border-border text-white rounded-xl px-4 py-2.5 outline-none focus:border-primary text-sm">
                {SHOW_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            ...
            <div>
              <label className="block text-sm text-muted mb-1.5">Regular Price (₹)</label>
              <input type="number" placeholder="e.g. 200" value={form.regularPrice} onChange={(e) => setForm({ ...form, regularPrice: e.target.value })} required min="1"
                className="w-full bg-dark border border-border text-white rounded-xl px-4 py-2.5 outline-none focus:border-primary text-sm" />
            </div>
            <div className="sm:col-span-2">
              <div className="flex items-center gap-2 cursor-pointer select-none" onClick={() => setForm({ ...form, hasGoldenSeats: !form.hasGoldenSeats })}>
            ...
            {form.hasGoldenSeats && (
              <>
                <div>
                  ...
                  <input type="number" placeholder="Min 30" value={form.goldenSeats} onChange={(e) => setForm({ ...form, goldenSeats: e.target.value })} required min="30"
                    className="w-full bg-dark border border-border text-white rounded-xl px-4 py-2.5 outline-none focus:border-primary text-sm" />
                </div>
                <div>
                  <label className="block text-sm text-muted mb-1.5">Golden Price (₹)</label>
                  <input type="number" placeholder="Must be > regular price" value={form.goldenPrice} onChange={(e) => setForm({ ...form, goldenPrice: e.target.value })} required min="1"
                    className="w-full bg-dark border border-border text-white rounded-xl px-4 py-2.5 outline-none focus:border-primary text-sm" />
                </div>
              </>
            )}
